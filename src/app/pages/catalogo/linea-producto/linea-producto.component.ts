...
export class LineaProductoComponent implements OnInit {
  ...
  constructor(
    ...
    ListarForm(id) {
      console.log("entro")
      for (let i = 0; i < this.lineaP.length; i++) {
        if (this.lineaP[i].id === id) {
          this.checkoutForm.setValue({
            descripcion: this.lineaP[i].descripcion
          });
        }
      }
    }
    ...
    Eliminar(id){
      this.dataService.EliminarLineaP(id).subscribe(resp=> {
        this.respuesta = resp;
        this.type = "success";
    ...
    Update(id){
       this.dataService.ActualizarLineaP(this.checkoutForm.value,id).subscribe(resp=> {
         this.respuesta = resp;
         this.type = "success";
         this.changeSuccessMessage(this.respuesta.msn)
         this.ngOnInit();
