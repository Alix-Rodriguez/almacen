...
@Component({
  selector: 'app-ordenes-cerradas',
  templateUrl: './ordenes-cerradas.component.html',
  styleUrls: ['./ordenes-cerradas.component.css']
})
export class OrdenesCerradasComponent implements OnInit {
  miga: any = 'Ordenes Cerradas';
  ...
  onSubmit() {
    ...
    this.dataRecepcion.saveOrdenesCerradas(this.checkoutForm.value)
      .subscribe(resp => {
        console.log(resp)
        this.respuesta = resp;
        this.type = "success";
    ...
    for (let i = 0; i < this.recepcion.length; i++) {
      if (this.recepcion[i].id === id) {
        this.checkoutForm.setValue({
          ...
          status: this.recepcion[i].status
  ...
  Eliminar(id: string) {
    console.log(id);
    this.dataRecepcion.eliminarOrdenesCerradas(id).subscribe(
      (resp) => {
        console.log(resp);
        this.respuesta = resp;
  ...
  listarRecep() {
    this.dataRecepcion.listarOrdenesCerradas().subscribe((resp) => {
      this.recepcion = resp['data']
    })
  }
  update(id: string) {
    console.log(this.checkoutForm.value);
    this.dataRecepcion.actualizarOrdenesCerradas(id, this.checkoutForm.value).subscribe(
      (resp) => {
        console.log(resp);
        this.respuesta = resp;
        this.changeSuccessMessage(this.respuesta.msn);
